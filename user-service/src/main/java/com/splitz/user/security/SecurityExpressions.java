 ...
 * Provides reusable authorization logic across controllers.
 */
@Component("security")
public class SecurityExpressions {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(SecurityExpressions.class);
     ...
     *
     * @param targetUserId The ID of the user being accessed/modified (accepts
     *                     Number to handle Integer/Long SpEL coercion)
    ...
    public boolean isOwnerOrAdmin(Number targetUserId) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("isOwnerOrAdmin invoked: principal={}, authorities={}, targetUserId={}",
                    auth.getPrincipal(), auth.getAuthorities(), targetUserId);
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("Ownership check: currentId={}, target={}, owner={}", currentId, targetUserId, owner);
        }
