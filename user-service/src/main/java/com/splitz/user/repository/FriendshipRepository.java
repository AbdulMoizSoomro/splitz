...
public interface FriendshipRepository extends JpaRepository<Friendship, Long> {
  ...
  /**
   * Finds all pending friend requests received by a user (where user is the addressee).
   *
   ...
   */
  Page<Friendship> findByAddresseeAndStatus(
      User addressee, FriendshipStatus status, Pageable pageable);

  /**
   * Finds all pending friend requests sent by a user (where user is the requester).
   *
   ...
   */
  @Query(
      "SELECT COUNT(f) FROM Friendship f WHERE (f.requester = :user OR f.addressee = :user) AND f.status = 'ACCEPTED'")
