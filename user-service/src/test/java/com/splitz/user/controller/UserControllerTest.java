...
/**
 * Unit tests for UserController focusing on request/response handling. Uses @WithMockUser with
 * ADMIN role for simplicity. Full security authorization is tested in UserControllerIntegrationTest
...
class UserControllerTest {
  ...
  private UserDTO createValidUserDTO(Long id) {
    return new UserDTO(
        id, "johndoe", "john@example.com", "John", "Doe", null // password not returned
        );
  }
  ...
  class GetUserByIdTests {
    ...
    void testGetUserById_WhenUserExists_ThenReturnsUser() throws Exception {
      ...
      // Act & Assert
      mockMvc
          .perform(get("/users/{id}", userId).contentType(MediaType.APPLICATION_JSON))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.id", is(1)))
          .andExpect(jsonPath("$.username", is("johndoe")))
          .andExpect(jsonPath("$.email", is("john@example.com")));
    ...
    @Test
    @DisplayName("Should return 500 INTERNAL SERVER ERROR when service throws exception")
    void testGetUserById_WhenServiceThrowsException_ThenReturnsInternalServerError()
        throws Exception {
      // Arrange
      Long userId = 1L;
  ...
  class UpdateUserTests {
    ...
    void testUpdateUser_WhenFirstNameTooLong_ThenReturnsBadRequest() throws Exception {
      ...
      UpdateUserDTO updateDTO =
          new UpdateUserDTO(longFirstName, "Doe", "john@example.com", "password123");
  ...
  class DeleteUserTests {
    ...
    void testDeleteUser_WhenUserExists_ThenReturnsNoContent() throws Exception {
      ...
      // Act & Assert
      mockMvc
          .perform(delete("/users/{id}", userId).contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    @DisplayName("Should return 500 INTERNAL SERVER ERROR when service throws exception")
    void testDeleteUser_WhenServiceThrowsException_ThenReturnsInternalServerError()
        throws Exception {
      // Arrange
      Long userId = 1L;
  ...
  class SearchUsersTests {
    ...
    void testSearchUsers_WhenUsersMatch_ThenReturnsPageOfUsers() throws Exception {
      ...
      when(userService.searchUsers(eq(query), any(Pageable.class))).thenReturn(page);
    ...
    void testSearchUsers_WhenNoMatches_ThenReturnsEmptyPage() throws Exception {
      ...
      when(userService.searchUsers(eq(query), any(Pageable.class))).thenReturn(emptyPage);
    ...
    void testSearchUsers_WhenPaginationParametersOmitted_ThenUsesDefaults() throws Exception {
      ...
      mockMvc
          .perform(
              get("/users/search").param("query", query).contentType(MediaType.APPLICATION_JSON))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.content", hasSize(1)));
    ...
    @Test
    @DisplayName("Should return 500 INTERNAL SERVER ERROR when service throws exception")
    void testSearchUsers_WhenServiceThrowsException_ThenReturnsInternalServerError()
        throws Exception {
      // Arrange
      String query = "john";
