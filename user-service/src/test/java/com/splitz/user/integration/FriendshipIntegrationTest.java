...
/**
 * Integration tests for Friendship endpoints covering the full flow: send request -> view pending
 * -> accept -> list friends -> remove.
 */
...
class FriendshipIntegrationTest {
  ...
  void friendshipEndToEndFlow() throws Exception {
    ...
    ResponseEntity<String> sendResponse =
    ...
    ResponseEntity<String> friendsResponse =
        restTemplate.exchange(
  ...
  void requesterCannotAcceptOwnRequest() throws Exception {
    ...
    ResponseEntity<String> bobAccept =
        restTemplate.exchange(
            baseUrl + "/users/" + bobId + "/friends/" + friendshipId + "/accept",
            HttpMethod.PUT,
            new HttpEntity<>(bobHeaders),
