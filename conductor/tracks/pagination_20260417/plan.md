...
## Phase 1: Setup & Testing Core Logic [checkpoint: 494756c]

- [x] Task: Write tests for `UserController.getAllUsers()` with pagination and hard limits.
- [x] Task: Write tests for `UserService.getAllUsers()` with pagination logic.
- [x] Task: Conductor - User Manual Verification 'Phase 1: Setup & Testing Core Logic' (Protocol in workflow.md)
...
- [ ] Task: Implement `InvalidPaginationException` and handle it in `GlobalExceptionHandler` (return 400).
- [ ] Task: Implement pagination validation in `UserController` (size limit 100, sorting allow-list: id, username, email, firstName, lastName, createdAt).
- [ ] Task: Set default sorting for `getAllUsers()` to `id,asc`.
...
- [ ] Task: Implement pagination validation for `searchUsers()` (consistent with `getAllUsers()`).
- [ ] Task: Set default sorting for `searchUsers()` to `username,asc`.
