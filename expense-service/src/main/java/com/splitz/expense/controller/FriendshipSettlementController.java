...
public class FriendshipSettlementController {
  ...
  public ResponseEntity<FriendshipSettlementDTO> createSettlement(
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(
            friendshipSettlementService.createSettlement(
                request, splitzAuthorizer.getCurrentUserId()));
  }
  ...
  public ResponseEntity<FriendshipSettlementDTO> getSettlement(@PathVariable("id") Long id) {
    return ResponseEntity.ok(
        friendshipSettlementService.getSettlementById(id, splitzAuthorizer.getCurrentUserId()));
  }
  ...
  public ResponseEntity<List<FriendshipSettlementDTO>> getSettlementsBetweenUsers(
    ...
    return ResponseEntity.ok(
        friendshipSettlementService.getSettlementsBetweenUsers(
            userId1, userId2, splitzAuthorizer.getCurrentUserId()));
  }
