...
test.describe('[E2E] Temporary Friends List', () => {
  test('should show temporary friends with group badges and allow request cancellation', async ({ browser }) => {
    const ts = Date.now();
    ...
    try {
      ...
      await expect(pageAlice.getByText(`@${bobName}`)).toBeVisible({ timeout: 10000 });
      await pageAlice.getByTitle('Remove Friend').click();
      await expect(pageAlice.getByText(/no friends added yet/i)).toBeVisible({ timeout: 5000 });

      // 7. Alice should now see Bob in the "Temporary Friends" list
      await expect(pageAlice.getByText('Temporary Friends')).toBeVisible({ timeout: 10000 });
      ...
      await expect(tempFriendCard.getByRole('button', { name: /cancel request/i })).toBeVisible({ timeout: 5000 });

      // 11. Alice cancels the request
      await tempFriendCard.getByRole('button', { name: /cancel request/i }).click();

      // 12. Verify "Add Friend" button returns
      await expect(tempFriendCard.getByRole('button', { name: /add friend/i })).toBeVisible({ timeout: 5000 });
