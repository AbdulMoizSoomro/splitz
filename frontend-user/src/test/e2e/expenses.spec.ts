...
test.describe('Expense Management', () => {
  test('should allow a user to create an expense with equal split', async ({ page }) => {
    ...
    await expect(expenseModal.getByText(/each person pays: \$45.00/i)).toBeVisible();
  ...
  test('should display exact split inputs and validation when toggled', async ({ page }) => {
    const timestamp = Date.now();
    ...
    await expect(expenseModal.getByText(/each person pays: \$100.00/i)).toBeVisible();
    ...
    const addButton = expenseModal.getByRole('button', { name: /add expense/i });
