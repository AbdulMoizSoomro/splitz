...
test.describe('[E2E] Friend Detail Page', () => {
  test('should navigate to friend detail page and show basic info, groups, and expenses', async ({ browser }) => {
    const ts = Date.now();
    ...
    try {
      ...
      await expect(pageAlice.getByRole('heading', { name: 'Bob User' })).toBeVisible();
