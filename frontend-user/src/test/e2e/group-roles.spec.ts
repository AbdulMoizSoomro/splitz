...
async function createGroupWithMember(
  ...
  await expect(friendPicker.getByText(/test user/i)).toBeVisible({
    timeout: 10000,
  });
  await friendPicker
...
test.describe("[E2E] Group Roles and Permissions", () => {
  ...
  test("Owner promotes a Member to Admin; new Admin sees Admin badge", async ({
    ...
    try {
      ...
      await expect(
        membersCard.locator("span", { hasText: "Member" }).first(),
      ).toBeVisible();
      await expect(
        membersCard.locator("span", { hasText: "Admin" }),
      ).not.toBeVisible();
      ...
      await expect(
        membersCard.locator("span", { hasText: "Member" }),
      ).not.toBeVisible();
      ...
      await expect(
        membersMemberCard.locator("span", { hasText: "Admin" }),
      ).toBeVisible({
        timeout: 10000,
      });
      ...
      await expect(
        membersMemberCard.locator("span", { hasText: "Owner" }),
      ).toBeVisible();
    } finally {
  ...
  test("Owner restricts member management; regular member cannot see role management controls", async ({
    ...
    try {
      ...
      const toggleBtn = pageOwner.getByLabel(
  ...
  test("Admin demotes another Admin back to Member", async ({ browser }) => {
    ...
    try {
      ...
      await expect(
        adminMembersCard.locator("span", { hasText: "Member" }).first(),
      ).toBeVisible({
        timeout: 10000,
      });
